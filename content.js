...
class PromptEnhancer {
  ...
  insertButton(inputElement, button) {
    ...
    if (hostname.includes("openai.com") || hostname.includes("chatgpt.com")) {
      ...
      // For Perplexity, try multiple insertion strategies
      let inserted = false;

      // Strategy 1: Find the closest parent with specific classes or attributes
      const parentContainer =
        inputElement.closest('[class*="input"]') ||
        inputElement.closest('[class*="editor"]') ||
        inputElement.closest('[class*="text"]') ||
        inputElement.closest("div");

      if (parentContainer && parentContainer.parentNode) {
        // Try to insert after the parent container
        const nextSibling = parentContainer.nextElementSibling;
        if (nextSibling) {
          parentContainer.parentNode.insertBefore(container, nextSibling);
        } else {
          parentContainer.parentNode.appendChild(container);
        }
        inserted = true;
      }

      // Strategy 2: If first strategy failed, try inserting directly after the input element
      if (!inserted && inputElement.parentNode) {
